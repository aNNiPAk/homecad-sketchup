...
- Distances and bbox dimensions are millimeters; area is square millimeters. Generic entity dimensions use `bbox_dimensions_mm`; the measurement kind is `bbox_dimensions`. Both report world-aligned axis-aligned bounding-box extents, not local or parametric dimensions. Rotation can increase these extents. The initial M1 names `dimensions_mm` and `dimensions` were ambiguous and are removed in 0.3.0; future HomeCAD domain parameters will be defined separately. Bounds and center distances use world-aligned boxes. Closest bounding-box distance is a geometric lower bound, not a collision test. `Face#area(transform)` and `Edge#length(transform)` account for parent scaling.
...
- SketchUp's native `Camera` wrapper is not safely copied by Ruby `clone`, as [SketchUp developers describe](https://forums.sketchup.com/t/how-can-i-save-and-restore-the-camera/241655). Capture reconstructs an independent camera from eye, target, up, projection, field of view or orthographic height, and aspect ratio. Restore uses [View#camera=](https://ruby.sketchup.com/Sketchup/View.html) with a zero-second transition and refreshes the view. `zoom_extents` frames [Model#bounds](https://ruby.sketchup.com/Sketchup/Model.html) through the same camera calculation, avoiding a separate viewport zoom operation. A changing capture with `restore_camera=true` rejects a two-point perspective camera because its extra offset cannot be restored through the documented camera setters.
- The official [Camera API](https://ruby.sketchup.com/Sketchup/Camera.html) exposes `fov_is_height?` as a getter; Ruby has no documented setter (the C API has a separate setter, which this extension does not call). Camera snapshots and reported/comparison state now include this flag. After constructing the snapshot and copying `aspect_ratio`, capture verifies that the reconstructed camera reports the same FOV orientation. If it does not, capture fails before changing the view. Regression tests cover perspective, parallel, current, top, iso, zoom-extents, target capture, exceptions, and the unsupported orientation case. This keeps the two-point perspective guard intact.
