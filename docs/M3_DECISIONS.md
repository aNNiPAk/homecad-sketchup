...
- Room's editable source of truth is `name` plus ordered `wall_ids`. Boundary coordinates, Room-facing sides, approximate area, Room relationships, and reference geometry are derived together from current wall geometry. `derive_room_state` and `room_relationships` are shared by Room creation, Room updates, and dependent Room regeneration after Wall updates.
- Room relationship keys are `wall_<uuid>` JSON keys, not direct `*_id` attributes; only hosted objects have a separate searchable `wall_id`. Replacing Room relationships therefore has no obsolete direct Room ID attributes to clean.
